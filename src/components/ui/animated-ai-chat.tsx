    ...
    return (
      <div className={cn("relative w-full", containerClassName)}>
        <textarea
          ref={ref}
          className={cn(
            "resize-none bg-transparent border-none outline-none focus:ring-0 focus:outline-none",
            className
          )}
...
export function AnimatedAIChat({
    ...
    commandSuggestions: propSuggestions,
    onSendMessage,
    isProcessing = false,
    ...
    const defaultCommandSuggestions: CommandSuggestion[] = [
        { 
            icon: <ImageIcon className="w-4 h-4" />, 
            label: "Analyze", 
            description: "Analyze portfolio or market data", 
            prefix: "/analyze" 
        },
        { 
    ...
    const commandSuggestions = propSuggestions || defaultCommandSuggestions;
    ...
    const showThinking = isTyping || isProcessing;
    ...
    return (
        <div className="min-h-screen flex flex-col w-full items-center justify-center bg-background text-foreground p-6 relative overflow-hidden">
            ...
            <div className="absolute inset-0 w-full h-full overflow-hidden">
                ...
                <div className="absolute bottom-0 right-1/4 w-96 h-96 bg-secondary/10 rounded-full mix-blend-normal filter blur-[128px] animate-pulse" style={{ animationDelay: '700ms' }} />
                <div className="absolute top-1/4 right-1/3 w-64 h-64 bg-accent/10 rounded-full mix-blend-normal filter blur-[96px] animate-pulse" style={{ animationDelay: '1000ms' }} />
            </div>
            ...
            <div className="w-full max-w-2xl mx-auto relative">
                ...
                >
                    ...
                    {/* Header Content (e.g., portfolio selector) */}
                    {headerContent && (
                        <motion.div
                            initial={{ opacity: 0, y: 10 }}
                            animate={{ opacity: 1, y: 0 }}
                    ...
                    >
                        ...
                        <div className="p-4 border-t border-border/50 flex items-center justify-between gap-4">
                            ...
                            <motion.button
                                ...
                                whileTap={{ scale: 0.98 }}
                                disabled={showThinking || !value.trim()}
                                className={cn(
                    ...
                    <div className="flex flex-wrap items-center justify-center gap-2">
                        {commandSuggestions.map((suggestion, index) => (
                            <motion.button
                                key={suggestion.prefix}
                                onClick={() => selectCommandSuggestion(index)}
                                className="flex items-center gap-2 px-3 py-2 bg-card/30 hover:bg-card/60 rounded-lg text-sm text-muted-foreground hover:text-foreground transition-all relative group border border-border/30"
                                initial={{ opacity: 0, y: 10 }}
                                animate={{ opacity: 1, y: 0 }}
            ...
            {inputFocused && (
                <motion.div 
                    className="fixed w-[50rem] h-[50rem] rounded-full pointer-events-none z-0 opacity-[0.03] bg-gradient-to-r from-primary via-accent to-secondary blur-[96px]"
                    animate={{
